...
class MountCheckoutStepsService
{
    ...
    public function handle()
    {
        ...
        if (isset($cppGateway->id)) {
            ...
            $cppCheckout = CppCheckout::updateOrCreate([
            ...
            // Step 1 -> Items do checkout ---
            /*$productsMounts = array_map(function ($product) {
                return [
                    'name'        => $product["name"],
            ...
            }, $model->order->products->toArray());
            $cppCheckout->step1()->updateOrCreate([
                "cpp_checkout_id" => $cppCheckout->id,
            ], [
                'items'   => json_encode($productsMounts),
                'visible' => true,
            ]);*/
